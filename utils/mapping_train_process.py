...
from utils.get_model import get_mapping_model
...
class GenomicModel(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
        #self.model = config.model_class(config.epi, config.label_model)
        self.model = get_mapping_model(config.model_class(config.epi, config.label_model),
                                       config.pretrained_model_path, config.label_model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(outputs, (tuple, list)):
            # outputs = outputs[0]
            outputs, _, epi_repr, cross_repr, epi, cross = outputs

        epi_loss = 1 - torch.nn.functional.cosine_similarity(epi_repr, epi, dim=-1).mean()
        cross_loss = 1 - torch.nn.functional.cosine_similarity(cross_repr, cross, dim=-1).mean()
        hic_loss = self.criterion(outputs, hic_target)
        loss = hic_loss + 0.5 * epi_loss + 0.5 * cross_loss

        # Log training loss
        self.log("train_loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=False)
        self.log("train_hic_loss", hic_loss, on_epoch=True, prog_bar=True)
        self.log("train_epi_loss", epi_loss, on_epoch=True, prog_bar=True)
        self.log("train_cross_loss", cross_loss, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_hic_loss", hic_loss, on_epoch=True, prog_bar=True)
        self.log("val_epi_loss", epi_loss, on_epoch=True, prog_bar=True)
        self.log("val_cross_loss", cross_loss, on_epoch=True, prog_bar=True)
...
def test_epoch(model, dataloader, criterion, device):
    ...
    avg_insu = np.mean(all_insu)
    avg_mse = np.mean(all_mse)
    avg_pear = np.mean(all_pear)
    avg_oe = np.mean(all_oe)
    avg_dis = np.mean(distance_stratified_correlation(all_preds_array, all_targets_array),axis=0)
