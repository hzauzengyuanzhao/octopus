...
def pearson_correlation(y_pred, y_true):
    ...
    for pred, true in zip(y_pred, y_true):
        ...
        true = np.array(true)
        # 计算皮尔逊相关系数
        corr, _ = pearsonr(pred.reshape(-1), true.reshape(-1))
        scores.append(corr)
    return scores
...
def distance_stratified_correlation(preds, targets):
    ...
    for pred, target in zip(preds, tqdm(targets)):
        ...
        for dis_i in range(len(pred)):
            ...
            metric, p_val = pearsonr(pred_diag_i.reshape(-1), target_diag_i.reshape(-1))
            distance_list.append(metric)
        scores.append(distance_list)
    results = scores
    return results

def virtual_4c_from_matrix(matrix, viewpoint_bp, seq_start, resolution=8192):
    viewpoint_bin = (viewpoint_bp - seq_start) // resolution
    if not (0 <= viewpoint_bin < matrix.shape[0]):
        raise ValueError("Out of matrix bounds")
    return matrix[viewpoint_bin, :]